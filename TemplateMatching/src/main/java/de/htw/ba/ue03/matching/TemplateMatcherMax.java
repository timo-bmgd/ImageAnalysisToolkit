 ...
 *
 */
public class TemplateMatcherMax extends TemplateMatcherBase {

	public TemplateMatcherMax(int[] templatePixel, int templateWidth, int templateHeight) {
			...
			for (int y = 0; y < distanceMap[x].length; y++) {
				...
				for (int tx = 0; tx < templateWidth; tx++) {
					for (int ty = 0; ty < templateHeight; ty++) {
						...
						double value = Math.abs(pixelImage - pixelTemplate);
						if(value>distance){
							distance = value;
						}
					}
